...
class CombinedLoss(nn.Module):
    def __init__(self, vocab = None, threshold=5.0, cross_entropy_weight=0.5, l1_l2_weight=0.5, weight = None, ignore_index=None):
        super(CombinedLoss, self).__init__()
        self.vocab = vocab
        self.threshold = threshold
        self.cross_entropy_weight = cross_entropy_weight  # 控制交叉熵损失的权重
        self.l1_l2_weight = l1_l2_weight  # 控制L1损失的权重
        self.weight = weight
        self.ignore_index = ignore_index

        # 定义交叉熵损失
        self.cross_entropy_loss = nn.CrossEntropyLoss(weight = self.weight, ignore_index=self.ignore_index)

    def forward(self, bbox_pred, bbox_target):
        # 计算交叉熵损失 (假设 pred 是 logits，target 是类别标签)
        cross_entropy_loss = self.cross_entropy_loss(bbox_pred, bbox_target)

        bbox_pred = bbox_pred.permute(0, 2, 1)

        # 获取每个位置概率最大的类别索引
        pred_indices = torch.argmax(bbox_pred, dim=-1)  # (batch_size, seq_len)
        
        # # 假设 vocab.id_to_token 可以通过该方法将 index 转换为 token
        # pred_tokens = [self.vocab.id_to_token(int(idx)) for idx in pred_indices.view(-1)]
        # pred_tokens = torch.tensor(pred_tokens, device=bbox_pred.device).view(bbox_target.shape)
        print(pred_indices[1], bbox_target[1])

        # 计算 L1 损失
        l1_loss = F.l1_loss(pred_indices.float(), bbox_target.float(), reduction='none')

        # 计算 L2 损失 (仅在 L1 损失较大时应用)
        loss = l1_loss
        if torch.mean(l1_loss) > self.threshold:
            loss = F.mse_loss(pred_indices.float(), bbox_target.float(), reduction='none')

        # 创建掩码，忽略填充位置
        mask = bbox_target != self.ignore_index
        
        # 应用权重：将权重应用到损失
        loss = loss * self.weight[bbox_target]  # 每个位置的损失乘以对应的权重
        ...
        # 合并交叉熵损失和 L1/L2 损失
        double_loss = self.cross_entropy_weight * cross_entropy_loss + self.l1_l2_weight * loss  
...
class TableTrainer:
    ...
    def train(
        ...
        if "bbox" in train_cfg.target:
            ...
            # )
            self.criterion_bbox = CombinedLoss(
                vocab = self.vocab,
                weight = torch.tensor(tmp, device=self.device),
                ignore_index=self.padding_idx,
            )
