...
from src.utils import pred_token_within_range, subsequent_mask, local_attention_mask
...
class Batch:
    ...
    def _prepare_transformer_input(
        ...
        #lcoal_mask = local_attention_mask(src.shape[-1]).to(self.device)
        casual_mask = local_attention_mask(src.shape[-1]).to(self.device)
