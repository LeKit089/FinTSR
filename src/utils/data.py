...
import sys
sys.path.append('/home/hj/data/zpl/unitable_second/')
print(sys.path)
...
def local_attention_mask(size: int, window_size: int = 300, pad: int = 0):
    attn_shape = (size, size)
    output = torch.ones(attn_shape).to(torch.bool)
    for i in range(size):
        for j in range(size):
    ...
    if pad and pad > 0:
        output[:pad] = False
    # 结合
    yg = subsequent_mask(size,pad)
    
    return torch.stack([output,yg])
...
def batch_autoregressive_decode(
    ...
    for _ in range(max_decode_len):
        ...
        #local_mask = local_attention_mask(context.shape[1]).to(device)
        ...
        with torch.no_grad():
            if use_ddp:
                logits = model.module.decode(
                    ...
                    #memory, context, tgt_mask=local_mask, tgt_padding_mask=None
                )
