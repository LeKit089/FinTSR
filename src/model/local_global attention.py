...
#         参数:
#         - embed_dim: 输入嵌入的维度 (d_model)
#         - num_heads: 多头注意力的头数
#         - dropout: Dropout 概率
#         - batch_first: 输入是否以 (batch_size, seq_length, embed_dim) 为顺序
#         """
#         super( ).__init__()
...
#         self.batch_first = batch_first  # 是否使用 batch_first 格式

#         # Query, Key, Value 的线性变换矩阵
...
#         实现缩放点积注意力
#         参数:
#         - Q: Query (batch_size, num_heads, seq_length, head_dim)
#         - K: Key (batch_size, num_heads, seq_length, head_dim)
#         - V: Value (batch_size, num_heads, seq_length, head_dim)
#         - attn_mask: 注意力掩码 (batch_size, num_heads, seq_length, seq_length)
#         - key_padding_mask: 填充掩码 (batch_size, seq_length)
...
#         """
#         # print(scores)
#         scores = torch.matmul(Q, K.transpose(-2, -1)) / (self.head_dim ** 0.5)  # 缩放
#         # 应用掩码
#         if attn_mask is not None:  # 用于控制注意力机制中的结构性屏蔽，例如防止未来信息泄露或限制注意力范围。
#             scores = scores.masked_fill(attn_mask == 1, float('1e-8'))

#         if key_padding_mask is not None: # 用于屏蔽填充的位置，确保模型在处理变长序列时不关注无效的填充部分。
#             scores = scores.masked_fill(key_padding_mask.unsqueeze(1).unsqueeze(2) == 1, float('1e-8')) # 广播机制，(batch_size, seq_len)->(batch_size, 1, 1, seq_len)，填充位置为1。
...
#         前向传播
#         参数:
#         - query: 输入的 Query (batch_size, seq_length, embed_dim) 或 (seq_length, batch_size, embed_dim)
#         - key: 输入的 Key (batch_size, seq_length, embed_dim) 或 (seq_length, batch_size, embed_dim)
#         - value: 输入的 Value (batch_size, seq_length, embed_dim) 或 (seq_length, batch_size, embed_dim)
#         - attn_mask: 注意力掩码 (batch_size, seq_length, seq_length) 或 (seq_length, batch_size, seq_length)
#         - key_padding_mask: 填充掩码 (batch_size, seq_length)

#         输出:
#         - output: 注意力后的输出 (batch_size, seq_length, embed_dim) 或 (seq_length, batch_size, embed_dim)
#         - attn_weights: 注意力权重 (batch_size, num_heads, seq_length, seq_length)
#         """
#         if not self.batch_first:
#             # 如果不是 batch_first，将输入维度调整为 (batch_size, seq_length, embed_dim)
...
#         _ , seq_length_k, _ = key.size()
#         K = self.k_proj(key).view(batch_size, seq_length_k, self.num_heads, self.head_dim).transpose(1, 2)
#         V = self.v_proj(value).view(batch_size, seq_length_k, self.num_heads, self.head_dim).transpose(1, 2)
...
#         if not self.batch_first:
#             # 如果不是 batch_first，将输出维度调整回 (seq_length, batch_size, embed_dim)
...
class DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        if self.norm_first:
            ...
            attn_output, _ = self.self_attn(x, x, x, attn_mask=tgt_mask[0], key_padding_mask=tgt_key_padding_mask)
            attn_output2, _ = self.self_attn2(x, x, x, attn_mask=tgt_mask[1], key_padding_mask=tgt_key_padding_mask)
            x = x + self.dropout(attn_output) + self.dropout(attn_output2)  # 残差连接
        else:
            # attn_output, _ = self.self_attn(x, x, x, attn_mask=tgt_mask, key_padding_mask=tgt_key_padding_mask)
            attn_output, _ = self.self_attn(x, x, x, attn_mask=tgt_mask[0], key_padding_mask=tgt_key_padding_mask)
            attn_output2, _ = self.self_attn2(x, x, x, attn_mask=tgt_mask[1], key_padding_mask=tgt_key_padding_mask)
...
class Decoder(nn.Module):
    ...
    def forward(
        ...
        # 两层manba
        for manba, norm in zip(self.manbas, self.norms):
            x = manba(norm(x))
            x = self.dropout(x)
        # 两层(结合注意力+不结合注意力）
        for layer, l2 in zip(self.layers, self.layers2):
            x = layer(x, memory, tgt_mask=tgt_mask, tgt_key_padding_mask=tgt_padding_mask)
            x = l2(x, memory, tgt_mask=tgt_mask, tgt_key_padding_mask=tgt_padding_mask)
        return x
