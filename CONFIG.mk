...
# batch size + epoch
BATCH18 = ++trainer.train.dataloader.batch_size=18 ++trainer.valid.dataloader.batch_size=18
...
VQVAE1M_WEIGHTS = $(MODEL_VQVAE) ++trainer.vqvae_weights="/home/hj/data/zpl/unitable_second/experiments/unitable_weights/vqvae_1m.pt"
VQVAE2M_WEIGHTS = $(MODEL_VQVAE_L) ++trainer.vqvae_weights="/home/hj/data/zpl/unitable_second/experiments/unitable_weights/vqvae_2m.pt"

# finetuning specific
WEIGHTS_mtim_1m_base = ++trainer.trainer.beit_pretrained_weights="/home/hj/data/zpl/unitable_second/experiments/unitable_weights/ssp_1m_base.pt"
WEIGHTS_mtim_1m_large = ++trainer.trainer.beit_pretrained_weights="/home/hj/data/zpl/unitable_second/experiments/unitable_weights/ssp_1m_large.pt"
WEIGHTS_mtim_2m_base = ++trainer.trainer.beit_pretrained_weights="/home/hj/data/zpl/unitable_second/experiments/unitable_weights/ssp_2m_base.pt"
WEIGHTS_mtim_2m_large = ++trainer.trainer.beit_pretrained_weights="/home/hj/data/zpl/unitable_second/experiments/unitable_weights/ssp_2m_large.pt"
...
# ----------------self 1 origin-------------------
LR_cosine20k_warm3k = $(LR_cosine) ++trainer.train.lr_scheduler.lr_lambda.total_step=20000 ++trainer.train.lr_scheduler.lr_lambda.warmup=3500 # epoch15，每轮 1266 steps

TRAIN_pub_part_91150_html := $(VOCAB_HTML) \
	$(PUBTABNET) $(LABEL_HTML) $(AUG_RESIZE_NORM) \
	$(TRAINER_TABLE) $(I448) $(SEQ512) \
	$(EPOCH24) $(OPT_ADAMW) $(OPT_WD5e2) $(LR_8e5)

EXP_ssp_2m_pub_part_91150_html_base_origin := $(TRAIN_pub_part_91150_html) $(ARCH_BASE) \
	$(WEIGHTS_mtim_2m_base) $(LOCK_MTIM_4) $(BATCH72) $(LR_cosine20k_warm3k)

# --------------评估-----------
HTML_RESULT_FILE = trainer.test.save_to_prefix=html
TEST_pub_part_91150_html := $(VOCAB_HTML) \
	$(LABEL_HTML) $(AUG_RESIZE_NORM) \
	$(TRAINER_TABLE) $(I448) $(SEQ512) $(HTML_RESULT_FILE)
EXP_ssp_2m_pub_part_91150_html_base_origin_eval := $(TEST_pub_part_91150_html) $(ARCH_BASE)
...
# ----------------self 1 origin-------------------
LR_cosine43k_warm6k = $(LR_cosine) ++trainer.train.lr_scheduler.lr_lambda.total_step=43000 ++trainer.train.lr_scheduler.lr_lambda.warmup=6500 # epoch15，每轮 2849 steps

TRAIN_pub_part_91150_bbox := $(VOCAB_BBOX) \
	$(PUBTABNET) $(LABEL_BBOX) $(AUG_RESIZE_NORM) \
	$(TRAINER_TABLE) $(I448) $(SEQ1024) \
	$(EPOCH24) $(OPT_ADAMW) $(OPT_WD5e2) $(LR_3e4) $(GRAD_CLIP12)

EXP_ssp_2m_pub_part_91150_bbox_base_origin := $(TRAIN_pub_part_91150_bbox) $(ARCH_BASE) \
	$(WEIGHTS_mtim_2m_base) $(LOCK_MTIM_4) $(BATCH32) $(LR_cosine43k_warm6k)

# ----------------self 2 unpackaged-------------------
LR_cosine76k_warm12k = $(LR_cosine) ++trainer.train.lr_scheduler.lr_lambda.total_step=76000 ++trainer.train.lr_scheduler.lr_lambda.warmup=12000 # epoch15，每轮 5064 steps

TRAIN_pub_part_91150_bbox_unpackaged := $(VOCAB_BBOX) \
	$(PUBTABNET) $(LABEL_BBOX) $(AUG_RESIZE_NORM) \
	$(TRAINER_TABLE) $(I448) $(SEQ1024) \
	$(EPOCH24) $(OPT_ADAMW) $(OPT_WD5e2) $(LR_3e4) $(GRAD_CLIP12)

EXP_ssp_2m_pub_part_91150_bbox_base_unpackaged := $(TRAIN_pub_part_91150_bbox_unpackaged) $(ARCH_BASE) \
	$(WEIGHTS_mtim_2m_base) $(LOCK_MTIM_4) $(BATCH18) $(LR_cosine76k_warm12k)

# ----------------self 3 unpackaged+local_attention-------------------
TRAIN_pub_part_91150_bbox_unpackaged_local := $(VOCAB_BBOX) \
	$(PUBTABNET) $(LABEL_BBOX) $(AUG_RESIZE_NORM) \
	$(TRAINER_TABLE) $(I448) $(SEQ1024) \
	$(EPOCH24) $(OPT_ADAMW) $(OPT_WD5e2) $(LR_3e4) $(GRAD_CLIP12)

EXP_ssp_2m_pub_part_91150_bbox_base_unpackaged_local := $(TRAIN_pub_part_91150_bbox_unpackaged_local) $(ARCH_BASE) \
	$(WEIGHTS_mtim_2m_base) $(LOCK_MTIM_4) $(BATCH18) $(LR_cosine76k_warm12k)

# ----------------self 4 unpackaged+mamba+local_attention-------------------
EXP_ssp_2m_pub_part_91150_bbox_base_unpackaged_local_mba := $(TRAIN_pub_part_91150_bbox_unpackaged_local) $(ARCH_BASE) \
	$(LOCK_MTIM_4) $(BATCH18) $(LR_cosine76k_warm12k)

# ----------------self 5 unpackaged+mamba+local_attention-------------------
EXP_ssp_2m_pub_part_91150_bbox_base_unpackaged_mba_local := $(TRAIN_pub_part_91150_bbox_unpackaged_local) $(ARCH_BASE) \
	$(WEIGHTS_mtim_2m_base) $(LOCK_MTIM_4) $(BATCH18) $(LR_cosine76k_warm12k)

# ----------------self 6 unpackaged+mamba+local_attention+dropout+Alter-------------------
EXP_ssp_2m_pub_part_91150_bbox_base_unpackaged_mba_local_alt := $(TRAIN_pub_part_91150_bbox_unpackaged_local) $(ARCH_BASE) \
	$(WEIGHTS_mtim_2m_base) $(LOCK_MTIM_4) $(BATCH18) $(LR_cosine76k_warm12k)

# ----------------self 7 unpackaged+mamba+local_attention+dropout-------------------
EXP_ssp_2m_pub_part_91150_bbox_base_unpackaged_mba_local_dpt := $(TRAIN_pub_part_91150_bbox_unpackaged_local) $(ARCH_BASE) \
	$(WEIGHTS_mtim_2m_base) $(LOCK_MTIM_4) $(BATCH18) $(LR_cosine76k_warm12k)

# ----------------self 8 unpackaged+mamba+local_attention+dropout-------------------
EXP_ssp_2m_pub_part_91150_bbox_base_unpackaged_mba_nvm_local_dpt := $(TRAIN_pub_part_91150_bbox_unpackaged_local) $(ARCH_BASE) \
	$(WEIGHTS_mtim_2m_base) $(LOCK_MTIM_4) $(BATCH18) $(LR_cosine76k_warm12k)

# ----------------self 9 unpackaged+mamba+comatten_attention+dropout-------------------
EXP_ssp_2m_pub_part_91150_bbox_base_unpackaged_mba_nvm_com_dpt := $(TRAIN_pub_part_91150_bbox_unpackaged_local) $(ARCH_BASE) \
	$(WEIGHTS_mtim_2m_base) $(LOCK_MTIM_4) $(BATCH18) $(LR_cosine76k_warm12k)
...
TEST_pub_part_91150_bbox := $(VOCAB_BBOX) \
	$(LABEL_BBOX) $(AUG_RESIZE_NORM) \
	$(TRAINER_TABLE) $(I448) $(SEQ1024) $(BBoX_RESULT_FILE)
EXP_ssp_2m_pub_part_91150_bbox_base_unpackaged_mba_nvm_com_dpt_eval := $(TEST_pub_part_91150_bbox) $(ARCH_BASE)
